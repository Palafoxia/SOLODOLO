...
public class Crawler implements Runnable {
    ...
    public Crawler(String seed, int threadNumber) {
        this.SEED = seed;
        ...
        if(level <= MAX_LEVEL && visited.size() < MAX_PAGES) {
            ...
            if(doc != null) {
                for(Element link : doc.select("a[href]")) {
                    ...
                    // If URL has not been visited and is not a badURL, check if malformed URL and crawl
                    if(!visited.contains(nextLink) && !badURLs.contains(nextLink) && nextLink != null) {
                        try {
                            assert nextLink != null : "Current URL contains no links ";
                            URL urlObj = new URL(nextLink);
    ...
    private String cleanURL(String url) {
        if(badURLs.contains(url)) { // Already is a bad URL
            ...
            // Remove index.html
            if(url.endsWith("index.html")) {url = url.substring(0, url.indexOf("index.html"));}
            
            // End after Login.aspx
            if(url.contains("Login.aspx")) {url = url.substring(0, url.indexOf("Login.aspx") + "Login.aspx".length());}
    ...
    private boolean isAllowed(String url) {
        ...
        try { // robots.txt exists
            ...
            website = urlObj.getProtocol() + "://" + urlObj.getHost();
            ...
            System.out.println("robots.txt doesn't exist for " + website);
            return true;
        }
    }
